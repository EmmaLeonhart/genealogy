...
| P1810 | subject named as | string — **qualifier on `P2600`**, carrying the name *Geni* renders. Emma, 2026-08-28: *"I want us to have the property P1810 with the specific name geni gives them."* Datatype and placement confirmed **offline** against `wikidata/items/`, where every `P1810` is a `string` qualifier sitting on an external identifier (`P396`, `P1280`, `P8034`, `P12458`). The value is `display_name` from `reports/display-names.csv`, never our own label — since 2026-08-29 that is the married form we chose, and the point of this property is what the *source* calls the person. **The redaction marker goes in VERBATIM, and this is the only place it may** — Emma's ruling, 2026-08-29, choosing the literal string over both the reconstructed `NN Garborg` and omitting the qualifier. `P1810` records what the source database *displays*, so `<private> Garborg` is the true value; her `mul` label stays `NN Garborg`. A **label** asserts what a person is called and a marker falsifies it; this asserts what Geni shows, and does not. `tests/test_garborg_day_batch.py` pins the line, permitting the marker on a `P1810` line and nowhere else. |

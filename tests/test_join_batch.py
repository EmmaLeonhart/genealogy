...
Asked on 2026-08-24 whether the join was implemented, the answer was no: 522,086
Geni ID ↔ QID pairs sat in a file nothing read. This is the consumer, and these are the
rules it must not break.

The rules are ruled, not invented here:
...
def test_the_geni_id_is_emitted_before_anything_derived_from_it():
    """The ordering rule, and it is about meaning rather than tidiness.
...
def test_every_derived_statement_cites_the_geni_profile():
    ...
    The Geni ID cannot cite itself, and `CLAUDE.md` records that the hand-made items
    never reference `P2600` — it *is* the reference.
    """

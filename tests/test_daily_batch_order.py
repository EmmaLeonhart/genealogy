...
`docs/daily-algorithm.md` § *Step 0* is two halves: read your Wikidata contributions into the
...
* **the summary keeps `missing` and `CONFLICT` and drops rows that are `extra` alone.**
  `extra` is your hand-work and is never touched; the full table is 90-odd rows of it, which
...
def test_the_summary_keeps_what_moves_and_drops_hand_work(daily):
    ...
    # Both of these are `extra` alone -- 22 siblings and 13 countries you added by hand.

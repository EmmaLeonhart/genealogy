# What a generated relationship label would say

Generated by `scripts/build-relationship-label-preview.py`. One row per placeholder person in `reports/relationship-label-preview.csv` — **39,440 people**.

Emma asked to see both populations before deciding whether the generated label runs for people who already carry a surname. The `mul` label is settled and is not the question: it stays `NN` or `NN <surname>`.

**Preview, not a final count.** Built from the derived CSVs, which predate the last 27 exports. The shape is what is being asked about.

| | bare `NN` | `NN` + surname |
| --- | ---: | ---: |
| people | 34,892 | 4,548 |
| a label can be generated | **28,674** (82%) | **3,208** (70%) |
| no relative with a real name | 6,218 | 1,340 |

## Which relative ends up naming them

| relation | bare `NN` | `NN` + surname |
| --- | ---: | ---: |
| father | 13,368 | 1,986 |
| mother | 2,185 | 106 |
| spouse | 5,564 | 677 |
| child | 929 | 104 |

## The rules applied here, and what they cost

Emma ruled on both of these on 2026-08-15 after seeing the first version of this preview.

1. **A redacted or placeholder relative is skipped**, and the precedence falls through to the next one — *"skip, fall through to the next relative"*. The first version put the marker into somebody else's label: *"husband of `<private>` Gaya Pereira"*, 2,730 times. Now **2** do. 12,409 people had a relative skipped and **7,422 of them (59%) still get a label** from a later relative. That is a minority: for the rest the skipped relative was the only one with a real name, so the skip costs the label outright. Every spouse and child is tried rather than only the first, which is what recovers the share that is recovered.
2. **A surname that is itself placeholder vocabulary collapses to bare `NN`** — `NN ???`, `NN NN`, `NN N.N.`, `NN Unknown`. 351 people moved from the surname population to the bare one, which is why the two totals here differ from the first version.

220 of the generable labels have **no recorded sex** and take the neutral form (`child of`, `spouse of`). Inventing a gender to make the label read better is not done here.


## Bare `NN` — what it would say

| `mul` | generated `en` | via | sex |
| --- | --- | --- | --- |
| NN | daughter of Angel Pereira Galdo | father | F |
| NN | daughter of Pedro Celín Gaya Rojas | father | F |
| NN | father of Monica Poveda Gaya | child | M |
| NN | son of Luis Fernando Gaya Pereira | father | M |
| NN | son of Carl Wilhelm Henry Key | father | M |
| NN | son of Kyriakos Savva Kylilis | father | M |
| NN | son of Kyriakos Savva Kylilis | father | M |
| NN | wife of Karl Ivar Hedin | spouse | F |
| NN | daughter of Johan Ivar Hedin | father | F |
| NN | wife of Tomas Mårtensson | spouse | F |
| NN | son of René Carlos Gaya Pereyra | father | M |
| NN | daughter of René Carlos Gaya Pereyra | father | F |
| NN | daughter of René Carlos Gaya Pereyra | father | F |
| NN | daughter of René Carlos Gaya Pereyra | father | F |
| NN | daughter of René Carlos Gaya Pereyra | father | F |
| NN | son of Rene Carlos Pereira Aranibar | father | M |
| NN | daughter of Rene Carlos Pereira Aranibar | father | F |
| NN | daughter of Rene Carlos Pereira Aranibar | father | F |
| NN | son of Osvaldo Krutsfeldt | father | M |
| NN | husband of Aída Pereira Aranibar | spouse | M |

## `NN` + surname — what it would say

| `mul` | generated `en` | via | sex |
| --- | --- | --- | --- |
| NN Budd | husband of Ann Ryves | spouse | M |
| NN 彭城武原 | daughter of Yu | father | F |
| NN Palaiologina | daughter of Konstantinos Angelos Comninos-Palaiologos Sebastokrator | father | F |
| NN Burgundians | son of Willibald of the Burgundians | father | M |
| NN de Roussillon | wife of Rostaing Arbald | spouse | F |
| NN d'Orléans | wife of Aubry de Mâcon | spouse | F |
| NN Hildesheim | daughter of Devorah Shapira | mother | F |
| NN Shmelkes | daughter of Menachem Mendel Shmelkes | father | F |
| NN van de Betuwe | daughter of Ricfried (Dodo) in de Betuwe | father | F |
| NN Olofsdotter | daughter of Olof Larsson | father | F |
| NN Sasani | daughter of Hormazd IV, King of Persia | father | F |
| NN de Salcedo (Ayala) | daughter of Ortún Sáenz Garcia de Salcedo VI Señor de Ayala | father | F |
| NN Calmans | daughter of Meyer Calmans | father | F |
| NN Nielsdatter (Hvide) | daughter of Niels Alexandersen Due | father | F |
| NN Johansdotter | daughter of Johan Jonsson Sr | father | F |
| NN Norton | husband of Jeanne de Brienne | spouse | M |
| NN Buonaparte | son of Carlo Maria Buonaparte | father | M |
| NN Kalf | son of Anders Kalf | father | M |
| NN Benkestok | son of Peder Benkestok Andersen | father | M |
| NN Benkestok | child of Trond VI Tordson Benkestok til Hananger | father | — |

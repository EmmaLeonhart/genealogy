...
Generated by `scripts/build-relationship-label-preview.py`. One row per placeholder person in `reports/relationship-label-preview.csv` — **39,691 people**.
...
| --- | ---: | ---: |
| people | 34,898 | 4,793 |
| a label can be generated | **28,679** (82%) | **3,450** (71%) |
| no relative with a real name | 6,219 | 1,343 |
...
1. **A redacted or placeholder relative is skipped**, and the precedence falls through to the next one — *"skip, fall through to the next relative"*. The first version put the marker into somebody else's label: *"husband of `<private>` Gaya Pereira"*, 2,730 times. Now **2** do. 12,411 people had a relative skipped and **7,423 of them (59%) still get a label** from a later relative. That is a minority: for the rest the skipped relative was the only one with a real name, so the skip costs the label outright. Every spouse and child is tried rather than only the first, which is what recovers the share that is recovered.
...
220 of the generable labels have **no recorded sex** and take the neutral form (`child of`, `spouse of`). Inventing a gender to make the label read better is not done here.
...
| NN 彭城武原 | daughter of Yu | father | F |
...
| NN van de Betuwe | daughter of Ricfried in de Betuwe | father | F |

...
from .quickstatements import Statement, geni_reference, render_statements
...
def render_quickstatements(batch: NameBatch) -> str:
    return render_statements(to_statements(batch))
...
def render_markdown(batch: NameBatch, *, top: int = 60) -> str:
    ...
    lines = [
        "# QuickStatements batch: name links",
        "",
        "Generated by `genimerge.namelinks` — re-run `python -m genimerge name-links`.",
        "",
        "**Nothing here has been sent to Wikidata.** `add-names.qs` is a file to",

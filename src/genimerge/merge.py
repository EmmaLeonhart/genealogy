        ...
        if base.value != other.value:
            self.report.conflicts.append(
                Conflict(
                    ...
                    path=path,
                    kept=base.value,
                    kept_from=self._origin_of(xref, path),
                    dropped=other.value,
                    dropped_from=self._source,
                )
            )
...
def merge_files(paths: list[str | Path]) -> tuple[Gedcom, MergeReport]:
    """Merge exports in the given order. Earlier files win value conflicts."""
...
def render_report(report: MergeReport, *, detail: bool = True, doc: Gedcom | None = None) -> str:
    ...
    if not report.conflicts:
        ...
        lines += [
            ...
            "The value from the earlier source was kept; the other is recorded here "

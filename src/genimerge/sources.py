...
def _post_merge_last(paths: list[Path], root: Path) -> list[Path]:
    ...
    **The design needs this and the obvious implementation does not provide it.**
    ...
    # Two exports can be seeded on the same person: one taken before a duplicate was
    # merged on Geni and one after. Path order puts them in an arbitrary order, and worse, a

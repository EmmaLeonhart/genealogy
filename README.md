...
- `reports/descendants.md` — the downward counterpart to `frontier.md`: people whose recorded descendant line is small but **not zero**, so the line demonstrably continues and we have barely followed it, ranked inside birth-year bands by how few generations down we have walked. The axis `density.md` does not have — it is about *when*, which is what the `Descendants` export campaign is for

...
`reports/geni-qid-links.tsv` holds the identifier written into these profiles
...
the correct representation, per `CLAUDE.md`. The merges are made by hand.
"""
import sys
import csv
...
def main():
    ...
    if dupes:
        print(f'\n{len(dupes)} roster people whose QID sits on more than one Geni profile '
              f'-- both ids emitted, merges are made by hand:')
        for r, ids in dupes:

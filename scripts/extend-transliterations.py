...
**A built batch came out with no Chinese or Japanese names on most of its people, and what was
there was inconsistent.** The readings have to come from a real pipeline.

`reports/garborg-name-transliterations.tsv` was **113 tokens
...
labels — **64 of 83 creations** in the batch under review, missing **90 distinct tokens**,
...
it contains. That inconsistency is the bug.
...
def by_rule(token):
    ...
    **This delegates now, and the version it replaced is why.** This file carried its own
    ...
    2026-08-25, when these readings turned out not to be coming from a real pipeline at all.
    The good engine was written and then never wired in: this module
...
def main():
    ...
    #
    # The token mappings are supposed to be MASSIVE: everything within two hops of anything
    # ever edited needs its tokenisation working correctly. The tokens are supposed to be made
    # programmatically rather than typed out one at a time.
    #
    # The machinery was already programmatic -- nothing was ever deleted,
    ...
    #
    # `--two-hops` is that scope: every person in the ledger, plus everyone within
...
def _placeholder_tokens():
    ...
    **The population the `ja`/`zh` gate actually stalls on.** `build-placeholder-label-batch.py`
    ...
    today's batch and `_two_hop_tokens` the two-hop neighbourhood; neither reaches here.
    """
    need = set()
...
def _two_hop_tokens():
    ...
    BOTH directions, because the scope is two hops either way and a parent is as much a
...
def _extend(rows, have, missing, args):
    ...
    # CJK was ruled to include Korean and the funnel began minting all three. `open(TABLE,

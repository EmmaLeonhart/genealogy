...
**You, 2026-08-25, looking at a built batch:** *"Also no Chinese or Japanese names for them? I am
...
You are right on all three counts. `reports/garborg-name-transliterations.tsv` was **113 tokens
...
labels — **64 of 83 creations** in the batch you were looking at, missing **90 distinct tokens**,
...
it contains. That inconsistency is the bug you were pointing at.
...
def by_rule(token):
    ...
    **This delegates now, and the version it replaced is why.** This file carried its own
    ...
    2026-08-25 after you asked *"did you kinda bullshit these instead of selecting from an
...
def main():
    ...
    #
    # You, 2026-08-29: *"these token mappings are supposed to be massive. They're supposed
    ...
    #
    # You are right that the machinery was already programmatic -- nothing was ever deleted,
    ...
    #
    # `--two-hops` is the scope you named: every person in the ledger, plus everyone within
...
def _placeholder_tokens():
    ...
    **The population the `ja`/`zh` gate actually stalls on.** `build-placeholder-label-batch.py`
    ...
    today's batch and `_two_hop_tokens` your neighbourhood; neither reaches here.
    """
    need = set()
...
def _two_hop_tokens():
    ...
    BOTH directions, because "two hops either way" is your phrase and a parent is as much a
...
def _extend(rows, have, missing, args):
    ...
    # You ruled that *"cjk includes korean"* and the funnel began minting all three. `open(TABLE,

...
Work out what in the repository is outdated: some things, the missing ancestors
among them, have gone stale without saying so.
...
def writes_in(text: str) -> set[str]:
    ...
    **`open(` alone was the bug.** The previous test counted any `open(...)` call containing a
    ...
    plain READ of a hand-maintained file — registered as an output. That deleted it from
    the script's inputs, which in turn defeated the reader-is-not-a-generator skip in
    `stale_against_inputs`, and the hand verdicts were reported as 35h behind

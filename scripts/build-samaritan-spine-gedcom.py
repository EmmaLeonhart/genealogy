...
every generation between them in descent order, one per record. That ordering,
instructed 2026-08-14, is what makes the file straightforward to
...
def build(people: list[tuple[int, dict]]) -> str:
    ...
    for i, (gen, rec) in enumerate(people, start=1):
        ...
        # file and Itamar the first, which is the ordering asked for.
        if i < len(people):

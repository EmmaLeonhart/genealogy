...
the per-language label is the generated relationship. Both were settled on
2026-08-15 after the preview was read, so this generates rather than asks.
...
`NN`, ruled over 351 people.

**`en` — the relationship label.** Precedence parent, spouse, child, giving
`daughter of Olof Larsson`, `wife of Rostaing Arbald`. Two rulings, both applied:
...
* **Everyone with a placeholder given name gets one**, including the 3,934 who
  ...
  inside the generated label. Shown the rows, the ruling was to generate: the label
...
name for 69% of bare-`NN` people but only 36% of `NN <surname>` ones. That reads
the opposite way to how it was first taken here: the surname ones being badly
connected is evidence *for* keeping the surname. For
...
English, Japanese, Chinese and `mul` are required on everything. `en` comes free
...
#: English, Japanese and Chinese on everything, plus `mul`. Ruled 2026-08-15.
REQUIRED = ("en", "ja", "zh")
...
#:
#: The in-law pair is new on 2026-09-01, by ruling. It is the largest single population:
...
CJK_RELATION = {
    ...
    # **Side-dependent: the Korean comes from `KO_BY_SIDE`, not from here.** The `NN` forms
    # are decided by logic rather than left blank. A nibling is reached

...
All Bure kinship people are wanted as random-walk start points, and the choice between roster
definitions was declined when it was offered — so both are built and the residue is the
deliverable.

**Why this is a different job.** Bure is a bunch of unlinked people carrying entity resolutions
to Geni, so it is not dense; it is a different kind of area needing its own algorithm. Because so
many of them already have Wikidata items, the QuickStatements this region wants are a different
shape and potentially more challenging.
...
def main():
    ...
    with open(R / "bure-roster.md", "w", encoding="utf-8", newline="\n") as f:
        ...
        f.write("Generated by `scripts/build-bure-roster.py`. Neither of the two definitions was "
                "chosen over the other, so both are built and **the residue is the "
                "deliverable**.\n\n")
        f.write("| | count |\n| --- | ---: |\n")

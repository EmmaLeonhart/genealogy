...
Asked for 2026-08-15, before a decision: check what this would actually look
like — the bare ones, the ones with surnames and the ones that properly have
relationships. Show both populations first and do
...
The precedence for choosing which relative names the person: **parent, father,
mother, spouse, child**. What is already settled and this does not re-ask:
...
#: exactly the normalisation that has been objected to.
CHILD_WORD = {"M": "son", "F": "daughter", "": "child"}
SPOUSE_WORD = {"M": "husband", "F": "wife", "": "spouse"}
PARENT_WORD = {"M": "father", "F": "mother", "": "parent"}

#: Two hops out. Queue item 6, and the ordering extends the one-hop
...
#: **The in-law pair, added 2026-09-01 by ruling.** They are not decoration: of the
...
#:
#: Grandparents, grandchildren and siblings were named in the long-range request and in-laws
#: were not; asked directly on 2026-09-01, they were added.
...
def is_unusable(label: str) -> bool:
    ...
    Two cases, both ruled on 2026-08-15: a redaction marker, which
...
#: Material that is not part of a name. Ruled 2026-08-19: trim it, and fix the English too.
...
def main() -> int:
    ...
    for row in family.values():
        ...
        # Ruled 2026-08-15: a redacted relative is SKIPPED and the precedence
        ...
        # placeholder ("husband of NN de Nantes", 53 cases). The redacted case was
        # ruled on and this one was not; it is the same shape,
        ...
        # One hop first, in that precedence. Two-hop candidates are appended
        ...
        sibs = siblings(gid)
        # **The side is known here and was being thrown away.** Raised 2026-09-01: the NN side
        # can simply be worked out. It can — a pibling is reached
        ...
        # Ruled 2026-08-15: a surname that is itself placeholder vocabulary
    ...
    lines = [
        ...
        f"**{len(rows):,} people**.",
        "",
        "Both populations were asked for before deciding whether the "
    ...
    # -- the rules set on this preview, and what they cost ------------------
    ...
    lines += [
        ...
        "",
        "Both of these were ruled on 2026-08-15 after seeing the first "

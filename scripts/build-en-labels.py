...
**The order, `queue.md` § LABELS:** *"makes en labels for every individual (so Japanese gets
...
**How to finish it:** no categorisation -- anything unclear is listed as `unknown` and the run
moves on. So there is no adjudication here. A person
...
items**. From CJK to English, no programmatic transliteration is attempted at all: they are
all bad. Nothing here transliterates — `pykakasi` and the Unihan

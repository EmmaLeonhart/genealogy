...
def main() -> int:
    ...
    for cut in THRESHOLDS:
        ...
        for rank, group in enumerate(clusters, 1):
            ...
            rows.append({
                ...
                "with_qid": sum(1 for n in group if ecc[rev[n]][1]),
    ...
    fields = ["cut", "rank", "people", "max_dist", "min_dist", "median_dist", "with_qid",
              "farthest", "farthest_geni"]
    ...
    for cut in THRESHOLDS:
        ...
        lines += [
            ...
            "",
            "| # | people | dist (min–max) | with QID | the farthest members |",
            "| ---: | ---: | --- | ---: | --- |",
        ]
        for r in got[:SHOW]:
            lines.append(f"| {r['rank']} | {r['people']:,} | {r['min_dist']}–{r['max_dist']} "
                         f"| {r['with_qid']} | {r['farthest']} |")
        if len(got) > SHOW:

"""Every turn you actually typed, across every transcript, newest first.
...
**Read BOTH record types, or the scan misses half of you.** A turn you typed while the model was
idle is `{"type": "user", "message": {"role": "user"}}`. A turn you typed while a tool call was
...
`role == "user"` scan finds 57% of what you said.

**What is dropped, and why each one is not you:**
...
#: Payloads that are the harness rather than you. Matched against the start of the text after
#: stripping whitespace, except `system-reminder` which can be preceded by your own words.
HARNESS_PREFIXES = (
    "<task-notification", "<local-command", "<command-name", "<command-message",
    "[Request interrupted", "Caveat: The messages below",
)

#: A cron prompt reaches the queue as an enqueue and is not you. These are the opening words of
...
def is_harness(text):
    ...
    if any(stripped.startswith(opener) for opener in CRON_OPENERS):
        return True
    # A compaction turn: narration reinserted at a context boundary, not something you wrote.

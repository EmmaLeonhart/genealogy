"""Every identification made by hand, in one file the pipeline reads each run.

    py scripts/build-manual-identifications.py

**The instruction, 2026-09-01:** *"I think we need to have the two identifications I did, and all
...
`zipper-sample` batch, all carrying a QID and a Geni id, every one a hand affirmation that
nothing acted on. `RIGHT` is the older word from before the deck settled on `SAME`; the fold
never learned it. Both are affirmative and both are hand verdicts.
...
1. **`reports/manual-identifications-extra.csv`** -- pairs given directly in conversation,
   which no deck produced. That is where the two Behm identifications live, and it is the file
   to append to when another is named.
2. **`reports/emma-judgments.tsv`**, verdict `SAME` or `RIGHT`.

A pair present in both keeps the extra file's note, because that is the typed one.
...
def main() -> int:
    ...
    # The extra file wins, because those were typed rather than clicked.
    if EXTRA.exists():

...
Queue item 2.D is to measure per property and assume no global winner — Geni
may be better at relationships and worse at dates — so nothing here decides a
winner, and the columns are deliberately raw.

The columns that are actually evidence:

``their_referenced``
    Whether Wikidata's disputed statement carries a reference. "Whichever
    cites a source" was refused as a *rule*, and it is still the strongest

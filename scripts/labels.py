...
**A redacted person is NOT left unlabelled either.** Ruled 2026-08-16: `NN` is always
preserved in the multi-language label, and descriptive labels for the relationships are
added in some languages on top of it. `NN` and `private` are the same thing here — a
private individual whose name is not exported comes out as an `NN`. So `Private` and `NN`
are one population reached by two routes and take one treatment:
...
**The person still goes in.** Ruled 2026-08-14: even where the data is affected by
redaction it is still informative and still belongs on Wikidata. The informative part is
...
exactly the material called valuable: these private names are worth including because
they flesh out the Wikidata side substantially.
...
#: **Only Geni's redaction markers, and only the two that were specified.**
#:
#: An earlier version of this set also held `nn`, `n n`, `unknown` and `?`, which was
#: refused on 2026-08-14: the NN people are not to be avoided. What was specified is
#: `Private` and `<private>`; the rest was added here unasked and silently suppressed
#: labels on people nobody had decided about.
#:
...
#
# The vocabularies were to end up as one, and there were
...
# **empties**, and that has been ruled on twice — `Private` and `<private>` and
...
#: Words meaning *unknown*, in any language. **Ruled 2026-08-17: words yes,
#: punctuation no.** Somebody who typed a word meaning *I don't know* is
...
WORDS_MEANING_UNKNOWN = {
    # **Stillborn is a DESCRIPTION, not a name.** Ruled 2026-08-30 on `Q141224141`: stop
    # assigning names to a person who has no names at all. Geni records him as `En dödfödd son Bielke` -- Swedish for *a stillborn son* --
    ...
    # `dödfödd` 116, `dödfött` 21, `dødfød` 4. The standing rule -- § *An obvious unknown-word
    ...
    "unknown wife",   #    37  A description of a relationship rather than a name,
                      #        and the ruling puts it here: not a `P735` given
    ...
    "未知",            #   204  Chinese, "unknown" — ruled 2026-08-18: `ukjent` and
    ...
    "某",              #   252  Chinese, "a certain one" -- the exact sense of `NN`.
                      #        Ruled 2026-08-19, asked whether it belonged here
                      #        beside 未知 and `ukjent`: it does. It is the whole
...
#: Punctuation, a marker **only as the whole label** — the other half of the
...
#: invented 283 of those (`f9b9f86`). Neither a word nor punctuation, so the
...
def is_placeholder_form(text: str) -> bool:
    ...
    **A label every component of which is a marker is a marker.** Ruled 2026-08-18,
    chosen over listing the spellings one at a time: the all-marker-components rule.
    Matching whole labels exactly had let `? ?` (13 rows),
...
#: reading as names, 4 records, found 2026-08-18 while checking the instruction that
...
def is_marker_label(label: str) -> bool:
    ...
    **Words yes, punctuation no** for the head test -- ruled 2026-08-17 -- so
...
def is_unnamed(gedcom_name: str) -> bool:
    ...
    a name, and it was settled on 2026-08-16 that the two get one treatment. It is
    **narrower than suppression**: an unnamed person is still created, still gets
    ...
    one. Nobody is dropped, which is the objection raised when `nn` was quietly
    added to `NOT_A_NAME`.
    ...
    **It consults the whole marker vocabulary, not `_NN_FORMS`.** It used to test
    ...
    It was settled on 2026-08-18, asked which of two such phrases counted: both are
    markers — alongside the earlier ruling that `ukjent` and 未知 get the `mul` NN
    treatment. That treatment is precisely this branch — `NN` in `mul`, a
    descriptive phrase in `en` — so routing these here is what the rule asks for,
    ...
    still created and still readable through `mul`. That distinction is the one whose
    loss was objected to when `nn` was quietly added to `NOT_A_NAME`.
    """
...
def labels_for(gedcom_name: str, descriptive: str = "") -> dict[str, str]:
    ...
    **A marker with a real surname behind it keeps the surname — in `mul` only.**
    ...
    they were reaching `en` verbatim, marker and all, on 575 rows. The split was chosen
    on 2026-08-18: a generated description in `en` instead, so
...
#:
#: **`Q141223436` was fixed by hand on 2026-09-04 and that edit is the specification.** The
#: report was only that the ordinals were wrong; the item now reads
...
#:
#: **Three separate conventions, all read off that edit rather than reasoned:** `ja` takes an
...
#: the 2026-08-27 ruling, and that is the right answer for them. Nothing in a rendered label separates a
...
def ordinal_readings(token, final=False):
    ...
    **`final=True` also accepts a bare `I` or `V`**, which `ORDINAL_RE` excludes on purpose
    ...
    the generation-suffix ruling of 2026-09-04 made `Sr.` into `I`: without it
...
def transliterate_token_ko(token, table, final=False):
    ...
    **`ko` is CJK.** Ruled 2026-09-01: Korean is as important as Chinese and is to be a
    standing priority, because CJK includes Korean. It had been filed with `hi`/`ar`/`ru`/`el` as a research task,
    ...
    # 2026-09-04.** The ruling of 2026-08-27 is that a letter is the same letter in every
    ...
    # A standing ruling must not be reachable only when the table happens to lack a row.
...
def transliterate_token(token, table, final=False):
    ...
    **An initial keeps its Latin letter in every language.** Ruled 2026-08-27, on what
    `John F. Smith` becomes in Japanese and Chinese: keep it Latin inside the label —
    ジョン・F・スミス, 约翰·F·史密斯. The alternatives offered and not taken were
...
def normalise_marker_spelling(label: str) -> str:
    ...
    **and nothing else**, while § *`NN` is PRESERVED in `mul`* has `Private` and `NN` as one
    population getting the same treatment. An attempt to settle that has been corrected twice,
    once sharply: the NN people are not to be avoided. So it stays a decision to be made rather
    than derived.
...
def is_description(label: str) -> bool:
    ...
    Three shapes, from the census — `queue.md` § *LABELS, IN ORDER*: an English or Spanish
...
def mul_for_description(label: str) -> str:
    ...
    **Ruled 2026-08-17:** `NN` for `mul` — plus the real surname where the description leaves
    one standing, the worked examples being `謝氏` → `NN 謝` and `信秀正室 織田` → `NN 織田`.
...
def strip_wedged_marker(label: str) -> str:
    ...
    **The instruction**, `queue.md` § *LABELS, IN ORDER*, the second of three marker
    populations: a real name with a marker wedged inside it has the marker stripped and the rest
    kept. `Catherine unknown` → `Catherine`, `Nechama (?) Heller` → `Nechama Heller`,
    `Hadaburg N.N. Gräfin im Saalgau` → `Hadaburg Gräfin im Saalgau`. Mechanical, no judgement.
    ...
    **It is the only one of the three that is mechanical**, because it decides nothing about what
    ...
    name slot — decide the label itself and are not mechanical.
...
def strip_markers(label: str) -> str:
    ...
    **Reported 2026-08-27** on `Q141198538`: it clearly has `nn` as its first name and was not
    produced as an NN person. Geni records her as `nn Gunnarsdatter /Frafjord/` — the marker
    ...
    **The marker stays.** `CLAUDE.md` § *`NN` is PRESERVED in `mul`* is explicit: `NN` is always
    preserved in the multi-language label, and descriptive labels for the relationships are added
    in some languages on top of it. Its worked example is exactly this shape:
    ...
    **A stillborn DESCRIPTION is removed whole, article and kinship word included.** Ruled
    2026-08-30 on `Q141224141`: stop assigning names to a person who has no names at all.
    Geni records him `En dödfödd son Bielke` — Swedish for
...
def name_with_unknown_surname(label: str, givn: str = "", surn: str = "",
    ...
    **Ruled 2026-09-07 on `Q141352187`:** a single given name is generally not acceptable and
    `Given NN` is strongly preferred — but he has a surname anyway. The item had gone out
    labelled `Ånon` and was corrected by hand to `Ånon Byre`, since Geni files `Byre` as his
    ...
    **The result is a `mul` label and nothing else.** Asked which languages carry what: `Given
    NN` for the `mul` label, with the `NN` replaced by prose in every language that is not
    ...
    **It also reconciles the ruling of 2026-08-29 rather than reversing it.**
    `drop_marker_surname` deletes the marker Geni put in `SURN`, on the rule that such a person
    is called by their first name — so the prose form `No name` never reaches a label — and this
    puts the marker back **normalised**: `Maria /No name/` is `Maria NN`, not `Maria No name`.
    That is the reading taken when the collision was put up for a decision.
...
def drop_marker_surname(label: str, *surnames: str) -> str:
    ...
    **Ruled 2026-08-29** on `Q141217396`, labelled *Maria No name*: such a person is called by
    their first name alone. So `Maria /No name/` is labelled
    ...
    **Why `is_marker_label` did not already catch it, because the gap is specific.**
    ...
    `no name` has been in `WORDS_MEANING_UNKNOWN` all along, which is what made it such an
    obvious case to have missed.

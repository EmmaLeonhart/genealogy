...
**The first version of `reports/eccentric-clusters.md` measured QIDs wrongly.** Both Chinese
lines likely have Wikidata items even with no connection recorded; pre-dynastic Egypt certainly
does, as do Axum and the Third Intermediate Period. Then: *"write these ones into that
identification gedcom thing that
...
constant, by design — *"Do not let it become an architecture"* — and its own docstring says
...
#: **No GitHub links anywhere** -- a repository URL in a constant names the repositories to
#: anyone reading the code, and so does a description of what the

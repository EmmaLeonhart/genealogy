...
**The order, `queue.md` § LABELS:** *"then mul gets made for every individual (almost always
...
* **An unnamed person keeps the MARKER in `mul`.** `CLAUDE.md` § *`NN` is PRESERVED in `mul`* —
  `NN` is always preserved in the multi-language label, and gains more descriptive labels in
  some languages for the relationships. So `mul` is `NN Garborg` while `en` is
...
`씨 /이/`). Anything unclear is listed as an unknown rather than categorised. They are listed.
    ...
    with io.open(DERIVED, encoding="utf-8", newline="") as fh:
        for r in csv.DictReader(fh):
            ...
            if mul and en and mul != en:
                ...
                # § *An obvious unknown-word marker goes straight in* draws the line:
                # **words yes, punctuation no**. `label_for` empties only `Private` and

...
**The order, `queue.md` § LABELS:** `en`, then `mul`, then **`ja`**, then `zh`, then the rest —
each one step over the whole population.

**The objection this file removes: katakana is what katakana is for.** The `ja` work had been
held back because a generated Japanese label would come out
...
`labels.transliterate_token` keeps as its Latin letter in every language — the 2026-08-27
ruling, ジョン・F・スミス.
...
**A Han-named person already has a `ja` label: the name as written.** If a name is written
solely in kanji, the Chinese and Japanese labels are the same string.
    ...
    with io.open(DERIVED, encoding="utf-8", newline="") as fh:
        for r in csv.DictReader(fh):
            ...
            if cjk and HAN.search(cjk):
                # The rule: a name written solely in kanji IS the Japanese label.
            ...
            for t in toks:
                ...
                # keeps a middle initial as its Latin letter and never changes case -- the

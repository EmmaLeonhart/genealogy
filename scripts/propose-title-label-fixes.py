...
* **`marker-only`.** `noble NN of Venne` reduces to a bare `NN`, and `CLAUDE.md` § *A BARE GIVEN
  ...
  distinguishing your from every other `NN`.
...
and only your explicit instruction of 2026-09-09 puts them in scope at all.
"""
import csv
import os
import sys
...
#: `CLAUDE.md` § *THE PARENT DECK* records your firm 2026-09-07 ruling that a CJK case is not
#: one you can adjudicate from where you are, so these are held rather than decided here.

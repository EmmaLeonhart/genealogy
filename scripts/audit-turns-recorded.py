"""Which instructions are not written down anywhere in the repo?
...
quotes them in fragments, and matching on any single common word matches everything. The phrase
taken is the longest run of words that is rare in the repo, which is what makes a hit meaningful
in both directions.

**A miss is a CANDIDATE, not a finding.** Instructions get repeated and rephrased, and much of
what is said is answered in the moment and needs no record. The output is a shortlist to read, and the
...
#: **And a turn longer than this is the HARNESS talking, not a typed turn.** Cron prompts,
#: skill bodies and pasted system text come through the same channel -- `CLAUDE.md` already says

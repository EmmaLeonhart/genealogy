...
You, 2026-08-15: *"For the synoptic tree, we're supposed to be specifically
...
same person unless something says otherwise — your 2026-08-12 rule: *"we merge
...
matcher you killed on 2026-08-12 and whose module was deleted on 2026-08-15.
...
#: your gate on everything created: `en` · `ja` · `zh` · `hi` · `ar` · `ru` ·
...
def is_placeholder_label(label: str) -> bool:
    ...
    **You, 2026-08-16:** *"NN is always preserved in the multi-language label. It
    ...
    **The head check defers to `labels.leads_with_a_marker`, and used to be its own
    ...
    markers on the strength of their punctuation. You ruled on that specific point
...
def label_set_for(row: dict) -> dict:
    ...
    **A placeholder was being created with `en` and nothing else.** Your spec is
...
def walk_all(anchors, fam, ourqid, names, label_sets, depth):
    ...
    You, 2026-08-16, on the three "questions" this had been sitting on:
    *"if they are only on wikidata there is no problem is there lol. But about
    ...
    SUPER EASY THING ANS YOU ARE TRATING IT AS A BLOCKER."* You are right on both.
    ...
    So this writes the two things you asked the midnight job to produce: the
            ...
            for prop, key in ((FATHER, "father"), (MOTHER, "mother")):
                ...
                if ours_id and theirs:
                    ...
                    if mine and mine in theirs:
                        ...
                        # missing from the output. You reviews cases one by one,
                        # so a row you cannot verify is not a case.
    ...
    #
    # That is not cosmetic. Your method is *"the structure picks the pair; the
...
def main() -> int:
    ...
    # `NN Hildesheim` sitting in `en`, which is the one placement your rule names.
    if PREVIEW.exists():

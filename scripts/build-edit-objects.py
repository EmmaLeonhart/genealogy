...
The specification of 2026-08-12 — see `edit-objects.md`. Four kinds:
`add_geni_id`, `create_individual`, `link_siblings`, `add_statement`.

The rules that shape the output, all ruled rather than invented here:
...
def label_slots(lab: dict) -> dict:
    ...
    `mul` carries `label_en`, not a marker. `NN` is always preserved in the
    multi-language label -- but the marker that belongs there is
...
def main() -> int:
    ...
    # Matches recorded by hand. **Nothing about these is special.** They
    # are Wikidata items matched to a Geni profile that do not yet state the Geni
    # ID — which is the ordinary output of a merge, and the ordinary starting
    # state for a person. It is just a Wikidata object; there is nothing special
    # about it.
    #
    ...
    # The spec: "creating an individual with all their names and such and linking

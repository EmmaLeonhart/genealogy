...
**The solo -> date -> name cascade was never a specified design.** Dates-then-names was ruled
about the **2x2 sibling** case specifically; it was generalised into an architecture and then
written up as though it had been asked for.

And the substantive half: a solo child says nothing unless there is some reason to match them.
...
The standard that governs all of it: these rules are empirical and the data has to be studied
empirically to decide what to make of them. Do not jump to conclusions on what sounds like it
might be true. Even parents is not certain.
...
* **independent corroboration** -- some source that is not the zipper puts the same pair together
  (the Geni About Me links, the hand identifications, the structural walk, the Izumo and Tanba
  rosters, the hand verdicts in `reports/emma-judgments.tsv`).
* **independent contradiction** -- such a source puts one of them with somebody else.

**The reliability order in `zipper-join.py`'s `SLOTS` is a stated ranking and is a hypothesis
this file exists to test.** Parents are said to be the most reliable, with the immediate warning
that even parents is not certain. Nothing here assumes the ranking is right or wrong; the table
is the answer.
...
def main():
    ...
    out.append("Generated by `scripts/measure-zipper-reliability.py`. **Nothing here is "
               "reasoned from what sounds right.** These rules are empirical: the data has "
               "to be studied empirically to decide what to make of them, rather than jumping "
               "to whatever sounds likely. Even parents is not certain.\n")

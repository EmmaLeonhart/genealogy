...
**⛔ STALE — this file was last audited 2026-08-16 and was flagged as probably stale on
2026-09-06.** It is. Measured that day, against the claims below:
...
birth/death dates, parent/child links, known-royalty name forms*. It was killed
on 2026-08-12 — a fuzzy matcher nobody consented to — and `correspondence.md`
states the rule: **no name similarity, ever.** `genimerge.reconcile`, which implemented
...
### 3z. The three phases (recorded 2026-08-04)
...
**Phase 2 — bulk (in progress).** Every export Geni will readily give is being taken,
**up to about 50**. These are not aimed at anything; they are cheap
...
**The descendants of the Indian people are the clearest example of a thin region.**
That is the first thing to check when the measure exists.
...
**"Region" means a region of the graph, not a place.** Do not classify people
geographically. Birthplace strings are dirty, most people
...
**Not before the bulk downloads are done.** Building this is wanted, and equally
clearly not yet — with ~50 exports still arriving, any
...
The project has since switched to exporting whatever Geni will readily give — 17
...
**This is NOT blocked on anybody and the tag saying so was wrong.** `CLAUDE.md`
§ *The batches are a SEQUENCE* lists "the merges/exports must wait" as one
...
GEDCOM, poll the download page, click through, with no human labour in it at all.
An export tagged BLOCKED-ON-USER-ACTION here is a thing nothing is blocking.
...
synoptic tree on the Geni id. **Thousands of small files, not aggregates** — that is the
signal, and it was missed once already.

**⛔ IT CANNOT RUN YET, and this is not a formality.** The scrape is to be done with the
extension, and only once the deliverables are coherently settled. Two conditions, both
currently unmet:
...
**The scheduler belongs in the todo, not the queue, and strictly speaking is never needed.**
It sat in `queue.md` as a blocker for most of a day and blocked nothing.
...
- **⛔ THE BACKGROUND SERVICE WORKER CANNOT BE RELOADED FROM HERE. It gates the SCHEDULER only.**
  ...
  **And it is not simply somebody clicking a button.** A phone has no
  `chrome://extensions` to reload from, and five automated routes were tried
  ...
  **The cause is almost certainly Chrome 137 removing `--load-extension`** unless re-enabled by
  policy, which is the most promising route. That is a registry change under
  `Software\Policies\Google\Chrome`, a security-relevant system setting, and is not one for
  this session to make.
...
**The format is JSON edit objects, not QuickStatements.** The 2026-08-12 spec
calls for JSON with dependency ordering, roughly a hundred executed per day;
QuickStatements was deleted entirely on 2026-08-15. Every batch below is JSON.
...
- **A NAME is Geni's display *label*, not always a name.** Some values
  ...
  a label or an alias, never a P735/P734 link. The names are not exactly the display
  names and not exactly the most natural ones — the pipeline has to decide per string
...
directly; the gap is the *English* label, which Wikidata often supplies. The plan
for the CJK cases (2026-08-07): translate to English where a romanisation is
missing, the harder part being to distinguish Japanese from Chinese first.
...
**The patronymics were as predicted.** All 633 Wikidata items that are
...
- **P2600 backfill** — `genimerge quickstatements` and its `.qs` output were
  **deleted 2026-08-15**. The work now lives in
  ...
  36 entries from the Wikidata URLs written onto Geni profiles by hand.
...
**Recorded 2026-08-05. Long-term — a comprehensive Geni export is still under way,
...
**Where exports pay, recorded because it changes the ranking.** Hyper-dense regions
are **low** return on an export: that material can come from Wikidata instead, and
where Geni disagrees with Wikidata there it will be over minor details rather than
whole people. Those regions are also expected to be *hard* to export from at all —
see the note on Geni entry points below. So the small-world but non-dense regions are the ones worth exporting,
...
Access to a cluster comes through *nearby contributions* — that is how
every export so far was seeded. Once an area is too densely covered by other
contributors no individual can be added to it, so the foothold an export needs
cannot be created. A larger export size limit would penetrate these
...
rather than a stalled item. Ask when it is actually next.

**The 2026-08-07 framing of order.life and the phase ordering.** order.life
is **a third source**, alongside Geni and Wikidata, feeding the Phase-4 queue of
things to add to Geni. It is **on a local PC** — not in this session's
reach — and it is **deferred deliberately** until the Wikidata side is built
offline. Because it is not Geni-native it needs a **different citation** from the
Geni-ID-as-source used for the Geni-derived claims. This does not change its
BLOCKED status; it names the block as a deliberate ordering choice rather than
just the other agent's edits.

The stated ordering, end to end: (1) descendant-distribution search to pick
...
**A postponed Geni-side merge queue — recorded, not started.** As of 2026-08-07:
...
are in place. It is explicitly **postponed until the Wikidata side is offline as
well** — the last thing, not a current task.

### 8a. How the Wikidata download must be built — the design constraints, 2026-08-07
...
**Treat it as a multi-day background operation, not a rush.** It is a couple-day-long
operation run in the background, **not** a do-it-as-fast-as-possible job that ignores
rate limits. Running in the background is what *makes it easy* — wall-clock length is not
...
line.** Expect HTTP 429s in quantity. The rule is
...
**Source: `chats/wikidata-querying-2026-08-07.md`.** § 8a above was taken to a
...
- **Commit and push in batches** — every 500–1000 items, or every few minutes,
  whichever comes first. Every hundred or every thousand individuals is fine;
  per individual is a big problem. Half a
...
- **Still hard-committed to the repo** — that part is settled and does not
  change. The cadence is the only thing being changed.
...
**Expansion scope — a prediction, recorded so it can be scored.** Expect
...
2. **What 500k full items weigh on disk, and what that does to a repo that must
   ...
   whether even that is viable. If it is not, the plan needs a decision about what
   "hard committed as part of the repo" means at this scale — not a quiet switch to
...
### 8a-decided. The calls of 2026-08-07 evening — this is the built design
...
**Live API for the whole seed set. The dump is not needed.** The 500,000 come easily
and come with all their data — 50 QIDs per `wbgetentities` request makes it ~10,300
requests, not 500,000, and the dump's cost was always the ~100 GB download the seed phase would
...
Not one file needing LFS — kept in a common way so the shards can be committed and
pushed as the run proceeds, building up the tree. Gzipped JSONL shards of
...
**Two queues, and the walk between them is the point.** The design:
...
**Reaching people with no Geni ID is the objective, not a side effect.** The walk
finds them and adds them in, and the end state is a large family tree including many
individuals with no GEDCOM id — who are the ones this is being done for. The two
trees are then parallel and unequal by construction — the
...
**The century distribution, Wikidata against Geni.** The guess made 2026-08-07:
the Geni-linked items on Wikidata skew heavily to the 20th and 21st centuries
much as the Geni profiles do, with the 19th ambiguous. Recorded as a
...
**Recorded 2026-08-14.** `order.life` is chaotic but is expected to **get the link
...
cannot merge them. It is expected to hold the correct father for him.

It is **not** a source to make our real-life tree from. It carries descent-from-
antiquity material that belongs **in the synoptic tree**, and its content is to be
gone over **later**.
...
**Moved out of `queue.md` on 2026-08-16**, because it is not work — it is a list of what
exists, and nothing in it waits on a date. The date is the 2026-08-14 rule: no Wikidata
edits until 1 September.
...
That file was made on 2026-08-15 because `todo.md` was untrusted; this audit is
...
(`reports/orderlife-properties.md` § *Genuinely novel*). The model for it, when
the time comes:
...
**The estimate of when: not soon.** The Cladoplast item will take a long time to be
made, so this is not currently relevant. Nothing is blocked on it.
...
**They are not a separate class of person** — everybody here is a human. So `P31` →
`Q5` stays, and no Gaiad-specific typing is emitted.
...
the reasoning that properties are `P`, and corrected on 2026-08-15: it is not `P`. So the
...
for `Q153802`, which is a raw substring search and was ruled out. It reads

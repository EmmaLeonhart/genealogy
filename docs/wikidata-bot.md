...
file. The login name is a secret too, not just the password.
...
**These are the three names set on 2026-08-14** (screenshotted from the
...
the account owner, and it was done on 2026-08-14.
...
account owner's to run, and they were run (or set through the web UI) on
2026-08-14.
...
rather than guessing at the batch format; the format question was settled by
...
- **Review before execute — load-bearing.** The sequence of edits is generated
  ...
  workflow executes *only* from that reviewed batch. The ordered sequence of edits
  in the repo is read before anything runs against Wikidata. Nothing
...
Set 2026-09-05: on the 15th all of this starts automatically, and what starts is
the daily Garborg batch, sent through the bot-password API.

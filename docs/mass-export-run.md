...
trusting it** — You spotted this from memory alone, asking why a campaign you thought was finished
...
The captcha that stopped the first attempt has not reappeared. You authorised the placeholder
...
You, 2026-08-28: *"Mass export these using our export protocol. Export from them and if their ids
...
Your 1am item, in your words: *"do a forest export and ancestor export on
...
**Geni `6000000040951562251`.** You supplied the profile URL after the search route dead-ended,
...
The pairing is now in `build-garborg-day.py`'s hard-coded `P2600` block, on your instruction:
...
**Export GEDCOM is not offered** on either — You manages neither profile. Seeded instead, tier 3:
...
Everyone in that file **except you**. You withdrew the Geni-bio-editing half of this item on
...
`6000000001846508982` → `Q232803` is **You and is skipped**.

### 4. The Bureätten people — COVERAGE of 251, not an export each

**You, 2026-08-28:** *"the bure people here we don't need to export from all of them we just
...
- **Never overwrite an existing `.ged`.** An export is named for its *style*, so filenames
  collide: append the seed id. If the destination path exists, **STOP** — where it goes is your
...
You asked for this specifically: *"if their ids were merged or moved record it each time and the
other stuff."*
...
Both kinds of move are the "weird stuff" you expect to find, and the point of writing them down
...
days to complete."* It is not a useful estimate. You: *"Forests take on average 6 minutes lol."*
...
`geni.com/pro/signup`, and the only ungated `/people/` href on a results page is your own
profile. You: *"geni search is a trap designed to upsell."* So search can confirm a person
...
existed to capture a Wikidata link in their Geni bio — which you withdrew when you moved that
injection into the synoptic tree build. Both the novelty and the purpose are gone.

**Group 4 needs 100 placeholder seeds, and you authorised them.** A direct profile URL works
without Pro — `geni.com/people/x/<id>` opened Nils Adolf Erik Nordenskiöld fine. But **Export
GEDCOM is absent from the Actions menu on a profile you do not manage**; that menu offers
Ancestor Report, Descendant Report and Merge This Profile and no export. So each of the 100 needs
the placeholder-seed technique: create a profile you manages in the target's neighbourhood, export
from it, and count success when the target appears in the result. That is 100 new Geni profiles,
which is yours to authorise and you did: *"And yes I'm authorizing new seed people on all of these
people lol"*.

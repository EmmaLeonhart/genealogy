# The parent walk — your algorithm, dictated 2026-09-05
...
**Your framing of the ordering:** *"I am giving you a very, very specific ordering of things."*
...
**The queue here is the ALGORITHM's queue.** Your words: *"just to be clear, this is the queue of
...
Your words, and the surname source is the part that changes what was built:
...
**SUGGEST SURNAMES IS ALWAYS ON.** You, 2026-09-05: *"suggested surnames are always a good
...
would be invented"*. Your ruling reverses that and the reasoning is yours: a plausible surname is
a better handle than none, and the patronymic-stripping rules above are what keep it honest.

**Matronymics are NOT supported.** Your words: if one existed it would go through all the
...
**Your reasoning, and it is the justification for mother-first:** *"mothers are most likely to be
...
**It is greedy best-first, and that is a deliberate change from the older method.** Your framing:
...
**Your words:** *"If it fails to add somebody for some reason, then it's pretty simple. It just
...
**You, 2026-09-05**, asked whether the walk should give parents to the placeholders it had just
...
people's trees. Two such proposals were sitting in front of a dry run when you were asked.

**The export that follows is `Forest` or `Descendants`.** Your words, same message: blood-relatives
...
**Your ruling, 2026-09-05**, asked who is created for somebody with no parents at all and no
...
**That divergence is SETTLED** -- see § *The zero-parent case takes the FATHER* above. You ruled on 2026-09-05 that a person with no parents and no patronymic gets the father, per the seed rules, and that the mother-first order governs the one-parent case.
...
`getImageData` — and your point stands that it should: *"the extension has to have eyes or it

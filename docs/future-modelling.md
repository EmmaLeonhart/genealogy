...
unblock it.** Created 2026-08-14 on request, alongside the batches that
...
| --- | --- | --- |
| **P59 Cladoplast of** | **Not mapped, not emitted.** See the section below — the property and the object are two different things and an earlier draft of this file conflated them. | the **object** existing on Wikidata, which is a decision not being made now |
...
**The Cladoplast property is not the Cladoplast object**, and an earlier draft of this
file conflated them. The property does not exist on Wikidata and probably never will. The
object might exist someday, and it would be strange for it to exist now.
...
- **The object** would be a Wikidata item for the concept *cladoplast*. That is
  the thing that might exist one day. It is not being made now.
...
wanted, it is a qualifier or a described statement, never an item. See
...
already have items. Named as one of the harder pieces from the outset. Not started.
...
link between Itamar and Shalma is the honest replacement. Not applied — which way to
take it is undecided.

## Silent drops in the relationship pass — measured 2026-08-14

**These were added by Claude without being asked, and were caught in review.** Both are
...
**`Q1` in the sex column is an error, and it is a fixable one** — not a third gender,
and clearly fixable on every item that carries it (2026-08-14). An earlier draft of this
...
Two ways out, and the choice between them is not made here:
...
**The general point, which matters more than either number.** Exception handling keeps
getting written for things that are not errors (2026-08-14). A skip that is never counted
reads as "there was nothing there". Every
...
straightforward. The call when the size was known: **skip it.** 80 claims on 20

# What a person item looks like, taken from the items actually built by hand

Read off `Q467497` and everything it links to. First captured 2026-08-22, re-read
2026-08-23, **re-read again 2026-08-24 against the live items** — these are being worked
through by hand and the shape is the template for the whole expansion programme, so this
file tracks what is actually being done rather than what a generator assumed.

## Where the hand-building has got to
...
batch left as a commented second pass are the ones being made by hand.
...
**The label is the name as used.** Geni holds `Stine "Stena" Eivindsdatter Garborg`; the
label is `Stena Eivindsdatter Garborg`. **This is not a shortening rule** — `Jon Eivindson
Garborg` keeps his full name. Geni wrapped the nickname in quotes and the nickname was
taken; where there is no nickname the name stands.
...
**Names are modelled, and this is the part a generator misses.** Eivind carries:
    ...
    P5056 patronym or matronym Q141152710 Aadnesson   (CREATED BY HAND)
...
on most existing patronymic items, and they are not added here.
...
- **Reference form** — it used `S854` + `S813`; the real items use `P2600`.
...
- **Argued against `P3373`** as redundant. The real items use it, both ways.
- **One-directional relationships** — `P22`/`P25` on children only, no `P40` on parents.
- **Suggested `en` descriptions.** The real items carry none.
...
`P5056` patronym item created by hand (`Q141152710` *Aadnesson*). Doing the same for the others
...
error this repo keeps paying for, so they are listed in the file's trailer rather than
generated.


## Re-read 2026-08-24 from the FULL items — and a retraction

**Download the full Wikidata items for the edited people to get the modelling; do not read
an edit history to see what is in them.**
...
**Every one of the eleven hand-created items carries exactly:** `P31`, `P21`, `P2600`,
...
**No hand-added qualifier appears anywhere.** No `P1545` *series ordinal*, no `P7452`
...
- **`Q467497` needs `P3373` to six more siblings** (he has Stena and Jon) and has no
  `P5056`.
- **No item carries `ja` or `zh`** except the three community ones, so the instruction to
  add them is untouched work.

## Measured again on 2026-08-26, over 48 hand-built items
...
whole items through `full_entities`, with `Special:Contributions/日巫女` separating what was
created by hand from what already existed.
...
Anything that proposes writing one is proposing something that has never once been done.
...
an item, so their share measures the neighbourhood's coverage rather than any intent.
...
**21 of the 33 `P1545` are on people with a single given name** — which is explicitly not
wanted: a series ordinal of 1 on a person with only one given name is one of the things the
batches kept including and nobody asked for. `scripts/namemodel.py` stopped emitting

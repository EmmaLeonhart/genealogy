...
Wikidata item, derived **one record at a time** from records have actually been
looked at. 2026-08-11: *"I want to go through all of the data modelling
...
**And this is ingestion, not conversion.** Same day: *"We are trying to do
...
- A row is **ESTABLISHED** only if it was stated, and the words are quoted.
- A row is **OPEN** if we have looked and not decided.
- A row is **TO ANALYSE** if it was assigned as research rather than a
  decision — the answer comes from the data, not from a ruling.
...
**ESTABLISHED 2026-08-11.** *"we are first and foremost doing all this
...
**The rule, verbatim:** *"we merge them based off of whether
...
- **No name similarity, ever.** Not as a tiebreak, not as corroboration, not as
  ...
  exactly what this forbids; it was ordered removed the same day and the whole
...
**ESTABLISHED 2026-08-11.** On Geni's day-precision date meeting
...
| B | Danureja I, Java | `6000000038363753264` | `Q97255794` | **bad example** — *"the Geni ID was made by somebody who has no fucking clue what they're doing, because the person is just too non-Western for the Western data stuff to actually apply"* |
| C | Aénor of Châtellerault, b. 1103 | `6000000000701127473` | `Q507801` | **bad example** — pre-modern, no real surname |

On choosing records: *"Realistically, probably people in the 1800s are the
best bet for this."*
...
**ESTABLISHED, from the specification rather than from us.** *"You realise
...
**ESTABLISHED 2026-08-11.** *"we are sorting by scripts. We are not sorting
...
The sketch of where it goes afterwards, recorded as intent and **not**
...
**OPEN.** The protocol named is to iterate the given names with the
...
**ESTABLISHED — it is the married name.** 2026-08-11: *"_MARNM is married
name."*

**ESTABLISHED — identical to `SURN` means ignore it.** *"if the married
...
**ESTABLISHED 2026-08-11.** *"if the surname is just a single dot, or the
...
**ESTABLISHED 2026-08-11 — do not strip it from a Latin-alphabet name.**
...
Stammerer` — which is not a suffix question at all. **The rule is not in
...
**ESTABLISHED 2026-08-11.** *"my vision here is that the English language
...
*"If there's only a name present in some sort of other script, we have to
...
**Latin names in several languages: not a problem we have measured.**
...
**And none of this is being emitted.** Same message: *"We're ingesting, and
...
**ESTABLISHED as easy.** *"The birth date and place are very easy things
...
**ESTABLISHED — `PLAC` only, `ADDR` ignored.** 2026-08-11, choosing
...
**ESTABLISHED — two properties, no qualifiers.** 2026-08-11: *"the date of
...
**ESTABLISHED — not used.** 2026-08-11: *"The note: we don't use notes
generally."*
...
**ESTABLISHED — ignored on the individual's own record.** 2026-08-11:
...
- **`RFN`** — restates the profile ID that `P2600` already carries.
...
- **22,513 families name one spouse with no children and no marriage event** —
  corpus-wide, not a quirk. These read as relationships Geni knows about
  whose partner was outside the export's scope.
- **16,229 of 36,257 dated `FAM` records name no spouse at all** (45%).
...
2026-08-11, repeatedly: the answer comes from the data and it is my job to
...
implausible. *"it's your job to do analysis on these to figure out what's
...
**Fix them in our data.** *"Fix them in the fucking data."* Five records —
...
**Ōjin and Wikramawardhana are ours to merge.** *"Fix them in our fucking
...
- **Creating new items is the point.** 2026-08-11: *"We create new people
...
- **Name items must be downloaded.** The store holds people; 0.4% of referenced
  name items are in it, so no name resolves offline. The choice made was a bulk
